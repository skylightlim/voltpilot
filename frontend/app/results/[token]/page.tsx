...
export default function ResultsPage() {
  ...
  // Seeded from the waiting page's handoff, so the finished result paints without
  // a loading flash. It cannot be seeded during render: the server has no
  // sessionStorage, so it renders the `if (!data)` loader while the client renders
  // the full page, and React discards the whole tree as a hydration mismatch.
  // Start where the server starts, then apply the handoff in a layout effect —
  // that runs after hydration but before the browser paints, so the loader still
  // never becomes visible. A direct link or a cleared session just fetches below.
  const [data, setData] = useState<any>(null);

  useLayoutEffect(() => {
    const cached = SESSION.loadResults(token);
    ...
    return (
      ...
      <ScrollRefresh />
        <div className="text-center">
          ...
          <p className="mt-3 text-[16px] text-muted">{t("r.compiling")}</p>
        </div>
      </main>
    );
  }
    ...
    <main className="app-shell mx-auto min-h-[100dvh] w-full max-w-2xl bg-background px-4 sm:px-5 pb-32">
      ...
      <header className="sticky top-0 z-30 -mx-4 sm:-mx-5 mb-8 border-b border-white/40 bg-background/65 px-4 sm:px-5 py-4 backdrop-blur-2xl supports-[backdrop-filter]:bg-background/40">
        <div className="flex items-center justify-between">
          ...
          <Link
            href="/"
            className="tap-target inline-flex items-center gap-1.5 rounded-full border border-border bg-white/50 px-3.5 py-2 text-[15px] font-medium text-muted transition-colors hover:border-primary/30 hover:bg-pine-tint/50 hover:text-ink"
          >
            {t("r.restart")}
          </Link>
        </div>
      ...
      {rec && top && (
        <ScrubReveal className="mb-10">
          <div className="group relative overflow-hidden rounded-lg bg-pine-deep text-white">
            {/* outer bezel */}
            ...
            {/* inner card */}
            <div className="relative m-[3px] rounded-[calc(24px-3px)] bg-gradient-to-br from-pine-deep via-pine-deep to-[#0a1f18] p-5 sm:p-8 shadow-[inset_0_1px_1px_rgba(255,255,255,0.06)]">
              ...
              <p className="mt-3 text-[16px] sm:text-[17px] leading-relaxed text-white/60 max-w-prose">
                {rec.summary}
              </p>
      ...
      {top && (
        <ScrubReveal className="mb-10">
            ...
            <div className="col-span-1">
              <div className="group relative overflow-hidden rounded-lg sm:rounded-lg border border-white/40 bg-white/70 p-4 sm:p-5 shadow-[0_1px_2px_rgba(0,0,0,0.04)] backdrop-blur-sm transition-all hover:border-emerald-500/20 hover:shadow-md">
                ...
                <p className="figure mt-1.5 text-[28px] text-ink leading-none">
                  <CountUp value={Math.round(top.co2_kg_yr)} /> <span className="text-[17px] text-muted font-normal">{t("r.kgyr")}</span>
                </p>
              </div>
            </div>
          ...
          {top.provenance && Object.keys(top.provenance).length > 0 && (
            <details className="group mt-4 rounded-lg border border-line bg-white/60 px-5 py-4">
      ...
      {data.solar && (
        <ScrubReveal className="mb-10">
          <div className="group relative overflow-hidden rounded-lg sm:rounded-lg border border-amber-300/40 bg-gradient-to-br from-[#fffbeb] via-[#fef3c7] to-[#fff9ec] p-4 sm:p-6 shadow-[0_1px_2px_rgba(0,0,0,0.03)] transition-all hover:shadow-md">
            <div className="absolute inset-0 rounded-lg ring-1 ring-inset ring-amber-500/[0.08]" />
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <p className="text-[16px] font-semibold text-ink">{t("r.solar")}</p>
                <p className="mt-1 text-[16px] leading-relaxed text-muted">
                  {(() => {
                    // Split the translated template on its placeholders so each
                    // figure keeps its emphasis wherever the language puts it.
                    const values: Record<string, string> = {
                      "{capex}": `RM${data.solar.capex_rm.toLocaleString()}`,
                      "{savings}": `~RM${data.solar.savings_rm_per_year.toLocaleString()}${t("r.perYear")}`,
                      "{payback}": t("r.solarPayback", { n: data.solar.payback_years }),
                    };
                    return t("r.solarBody")
                      .split(/(\{capex\}|\{savings\}|\{payback\})/)
                      .map((part, i) =>
                        values[part] ? (
      ...
      <section className="space-y-2.5 sm:space-y-3">
        ...
        >
          ...
          <div className="mt-4 overflow-x-auto -mx-1 px-1">
            {/* The badge used to sit inline after the model name, so it started at a
                different x on every row, and 440px squeezed long names onto three
                lines. Its own column aligns the badges and lets the name hold one
                line; the wrapper scrolls horizontally when the viewport is narrower. */}
            <table className="w-full min-w-[620px] border-collapse text-[15px]">
              <thead>
                <tr className="text-left text-[11px] uppercase tracking-[0.1em] text-muted">
                  <th className="pb-3 pr-3 text-right font-semibold">#</th>
              ...
              <tbody>
                {filteredRanking.map((r: Row) => (
                  ...
                  >
                    <td className="py-2.5 pr-3 text-right align-middle tabular-nums text-muted">
                      {r.rank}
                    </td>
                    <td className="whitespace-nowrap py-2.5 pr-3 align-middle text-ink">
                      {r.brand} {r.model}
                    </td>
                    <td className="py-2.5 pr-3 align-middle">
                      <Badge tone={r.type === "ev" ? "ev" : "hybrid"}>
                        {r.type === "ev" ? "EV" : "Hybrid"}
                      </Badge>
                    </td>
                    <td className="whitespace-nowrap py-2.5 pr-3 align-middle text-muted">
                      {r.body || "—"}
                    </td>
                    <td className="py-2.5 pr-3 text-right align-middle tabular-nums text-ink">
                      {r.topsis_score.toFixed(3)}
                    </td>
                    <td className="whitespace-nowrap py-2.5 pr-3 text-right align-middle tabular-nums text-ink">
                      RM{r.price_rm.toLocaleString()}
                    </td>
                    <td className="whitespace-nowrap py-2.5 text-right align-middle tabular-nums text-muted">
                      RM{r.running_cost_rm_yr.toLocaleString()}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <p className="mt-4 text-[12px] leading-relaxed text-muted">
            <b className="text-ink">{t("r.energyNoteLabel")}</b> {t("r.energyNote")}
          </p>
        </DashboardCard>
                ...
                return (
                  <div key={key} className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="figure text-[16px] text-ink">{value}</span>
                    </div>
        ...
        >
          <div className="mt-4">
            ...
            />
            <div className="mt-4 flex items-start gap-3 rounded-lg border border-amber-300/40 bg-amber-50/60 p-4">
              ...
              <p className="text-[15px] leading-relaxed text-ink">{batteryWarning.text}</p>
            </div>
            <button
      ...
      {showPdf && (
        ...
        >
          <div
            className="w-full max-w-md rounded-t-lg sm:rounded-lg border-t sm:border border-white/20 bg-white/95 p-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] sm:pb-6 shadow-2xl backdrop-blur-xl enter-rise"
      ...
      {/* ── PDF Sent Toast ────────────────────────────────────────────── */}
      {pdfSent && (
        <div className="fixed inset-x-4 bottom-[calc(1rem+env(safe-area-inset-bottom))] z-50 flex items-start gap-3 rounded-lg bg-pine-deep px-5 py-4 text-white shadow-2xl shadow-black/20 enter-rise">
          ...
          <div>
            ...
            <p className="text-[15px] text-white/60">The PDF went to {pdfSent.email}.</p>
          </div>
        </div>
      )}
    </main>
  ...
  return (
    <div className="overflow-hidden rounded-lg sm:rounded-lg border border-white/40 bg-white/70 backdrop-blur-sm shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition-all hover:shadow-md">
      ...
      >
        ...
        <span className="text-[16px] font-semibold text-ink flex-1">{label}</span>
